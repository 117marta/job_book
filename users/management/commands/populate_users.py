...
class Command(BaseCommand):
    ...
    def _populate_users(self, role, birth_date=None, trade=None, trades=None):
        if birth_date:
            birth_date = fake.date_of_birth(minimum_age=self.MINIMUM_AGE, maximum_age=self.MAXIMUM_AGE)
    ...
    def handle(self, *args, **options):
        ...
        self._populate_users(role=SITE_MANAGER, birth_date=True, trades=(railway, road, construction))

        # Create site managers and site engineers
        for trade in ALL_TRADES:
            self._populate_users(role=CLERK_OF_THE_WORKS, birth_date=True, trade=Trade.objects.get(abbreviation=trade))
            self._populate_users(role=SITE_ENGINEER, birth_date=True, trade=Trade.objects.get(abbreviation=trade))
        ...
        for _ in range(self.NUMBER_OF_SUBCONTRACTORS):
            ...
            self._populate_users(role=SUBCONTRACTOR, trade=Trade.objects.get(abbreviation=choice_trade))
